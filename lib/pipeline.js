...
export async function step2_write(geminiOutput) {
    ...
    // 방어 로직 2: 필수 데이터(article.title, article.deepdive) 누락 시 에러 발생시켜 재시도 유도
    if (!parsedData || typeof parsedData !== 'object') {
        throw new Error(`Claude 원고 데이터가 유효한 객체가 아닙니다.`);
    }
    if (!parsedData.article || !parsedData.article.title || !parsedData.article.deepdive) {
        const missing = [];
        if (!parsedData.article) missing.push('article');
        else {
            if (!parsedData.article.title) missing.push('article.title');
            if (!parsedData.article.deepdive) missing.push('article.deepdive');
        }
        throw new Error(`Claude 원고에 필수 데이터가 누락되었습니다: ${missing.join(', ')}. 전달된 최상위 키: ${Object.keys(parsedData).join(', ')}`);
    }
