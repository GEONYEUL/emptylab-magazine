...
export async function step2_write(geminiOutput) {
    ...
    // max_tokens를 8192로 증가: 4096에서는 복잡한 JSON 구조(6개 섹션 + SNS + 해시태그)가
    // 토큰 한도에 걸려 중간에 잘리면서 불완전한 JSON이 반환되어 파싱 실패하는 케이스 방지
    ...
    console.log(`[Claude 글쓰기] 응답 길이: ${responseText.length}자, stop_reason: ${message.stop_reason}`);

    // stop_reason이 'end_turn'이 아니면 토큰 한도에 걸려 잘린 것 → 경고 로그
    if (message.stop_reason !== 'end_turn') {
        console.warn(`[Claude 글쓰기] ⚠️ stop_reason이 '${message.stop_reason}' — 응답이 잘렸을 수 있음`);
    }
