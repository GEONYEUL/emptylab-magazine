...
export async function step2_write(geminiOutput) {
    ...
    // LLM이 구조를 어떻게 망가뜨려도(이중 감싸기, 키 이름 변경 등) 값을 찾아냄
    const findField = (obj, keyName) => {
        if (!obj || typeof obj !== 'object') return null;
        if (obj[keyName] && typeof obj[keyName] === 'string' && obj[keyName].trim().length > 0) return obj[keyName];
        for (const k of Object.keys(obj)) {
            const found = findField(obj[k], keyName);
