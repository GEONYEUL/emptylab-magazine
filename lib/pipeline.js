...
export async function step1_preprocess(articles, keyword = null) {
    ...
    promptWithData = promptWithData.replace(
        '{{USER_KEYWORD}}',
        keyword ? `"${keyword}"` : "지정된 키워드 없음 (오늘의 전체 트렌드 분석)"
    );
    ...
    // Gemini 응답이 빈 문자열이거나 파싱 불가능한 경우 안전 처리
    try {
        const geminiOutput = JSON.parse(responseText);
        if (geminiOutput.error) return geminiOutput;
        return geminiOutput;
    } catch (e) {
        console.error('[STEP 1] Gemini JSON 파싱 실패. 원본 응답:', responseText.substring(0, 300));
        throw new Error(`Gemini 전처리 JSON 파싱 실패: ${e.message}`);
    }
}
