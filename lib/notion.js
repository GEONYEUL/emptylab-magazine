...
export async function saveToNotion(parsedData) {
    ...
    // 카드뉴스 대본 속성 추가 (구조화된 객체 배열 or 문자열 배열 모두 지원)
    ...
    // 구조화된 객체를 텍스트로 변환하는 헬퍼
    const formatCardSlide = (slide) => {
        if (typeof slide === 'string') return slide;
        if (typeof slide === 'object' && slide !== null) {
            let result = `[${slide.slide || '?'}] ${slide.type || ''}: ${slide.text || ''}`;
            if (slide.image_guide) result += `\n🎨 이미지 가이드: ${slide.image_guide}`;
            return result;
        }
        return JSON.stringify(slide);
    };
    const cardNewsText = cardNewsArr.map(formatCardSlide).join('\n\n').substring(0, 2000);
    if (cardNewsText) {
    ...
    if (cardNewsArr.length > 0) {
        children.push(
            ...
            ...cardNewsArr.map(s => ({
                ...
                numbered_list_item: {
                    rich_text: [{ text: { content: formatCardSlide(s) } }]
                }
            }))
        );
    }
