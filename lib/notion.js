...
export async function saveToNotion(parsedData) {
    ...
    const properties = {
        ...
        "참고 출처": { multi_select: (meta.sources_used || []).map(s => {
            const sourceName = typeof s === 'string' ? s : s.source;
            // Notion select 태그명은 너무 길면 안 되므로 매체명만 추출
            return { name: sourceName || 'Unknown' };
        }) },
    };
    ...
    if (meta.sources_used?.length > 0) {
        children.push({
            ...
            toggle: {
                ...
                children: meta.sources_used.map(s => {
                    if (typeof s === 'string') {
